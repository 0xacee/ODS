...
class SystemdAccessBridge:
    def __init__(self, install_dir, edge_key, *, state=STATE, dropin=DROPIN, installed_binary=None, gateway_owner=None, settings_data_dir=None, gateway_binding=None):
    ...
    def verify_host_agent_custody(self):
        ...
        values = self.command(['systemctl', 'show', 'ods-host-agent.service', '--property=LoadState,User,MainPID'])
        ...
        try:
            ...
            raise AccessError('root-host-agent-isolation-required') from None
        if b'-I' not in args:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                with connection.getresponse() as response:
                    if response.status != 200: raise AccessError("runtime-unavailable-or-busy", http_status=response.status)
                    raw = response.read(65537)
