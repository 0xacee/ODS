...
_ods_pixel_restart_gateway_and_verify() {
    ...
    for attempt in {1..60}; do
        if _ods_pixel_gateway_health; then
            ready=true
            break
        fi
        (( attempt < 60 )) && sleep 2
...
_ods_pixel_install_access_service() {
    local owner="$1" openclaw_bin="$2" adopt_existing="${3:-false}" activate="${4:-true}"
    ...
    ods_sudo python3 - "${INSTALL_DIR:?}" "$owner" "$openclaw_bin" "$adopt_existing" "$activate" <<'PY'
import fcntl, hashlib, json, os, pathlib, pwd, stat, subprocess, sys, tempfile
...
write(config_dir / 'pixel-access.json', json.dumps({'install_dir': str(source.resolve()), 'owner': owner.pw_name,
    'openclaw_bin': str(binary), 'settings_data_dir': settings_data_dir,
...
_ods_pixel_gateway_health() {
    ...
    for host in '[::1]' '127.0.0.1'; do
        if body="$(curl --noproxy '*' --fail --silent --show-error --max-time 2.5 \
            "http://${host}:18789/health" 2>/dev/null)" \
...
_ods_pixel_wait_gateway() {
    ...
    for ((attempt = 1; attempt <= attempts; attempt++)); do
        _ods_pixel_gateway_health && return 0
    ...
    if [[ "$reuse_active" == true ]]; then
        ...
        if [[ "$same_verified_source" == true ]]; then
            ...
            if [[ "$same_source_resume" == true ]]; then
                ...
                if ! _ods_pixel_wait_gateway 60; then
