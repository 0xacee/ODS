...
For a new project without a requested destination, use a descriptive folder under Playground. Keep existing projects in place. A dev server inside the sandbox is not a browser-accessible URL. For supported static publication, run the framework's real build and use pixel_ods_workspace_preview with its output directory containing index.html. Never handwrite dist files to imitate a successful compiler. Preview URLs include a site path prefix: build with relative asset URLs when supported (for Vite, --base=./), rather than assuming /assets resolves to this project. Inspect the actual output and verify its scripts/styles load and the application boots at the exact published URL. Preserve the source framework; index.html may exist only in build output. Share the exact verified publication URL. Static HTTP readback proves publication, not interactions; exercise requested behavior with an available browser capability.`,
