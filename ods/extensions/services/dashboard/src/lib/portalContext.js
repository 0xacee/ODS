...
export function usePortalContext({chatId,runtimeKey,capacity,initialRequestId,blocked,onPendingChange}) {
  ...
  const generation=useRef(0),lastQuery=useRef(0),priorRuntime=useRef(runtimeKey),inspected=useRef(false)
  ...
  current.current={chatId,runtimeKey,capacity,blocked,onPendingChange,historyUnknown:view.chatId===chatId && view.runtimeKey===runtimeKey && view.historyUnknown}
  ...
  const accept=useCallback((snapshot,fromMutation=false)=>{
    ...
    const phase=restarted?'interrupted':missing?'skipped':unavailable?'unavailable':elsewhere?'busy':operation.status==='running'?'running':terminal && manual?operation.status
      :snapshot.status==='busy'?'busy':pending.current?'unknown':'idle'
    const measurement=snapshot.context?JSON.stringify([snapshot.context.used,snapshot.context.window,snapshot.context.measuredAt || null]):null
    const context=snapshot.status==='ready' && snapshot.context
      && (!snapshot.model || snapshot.model.contextWindow===snapshot.context.window)
      && measurement!==invalidatedMeasurement.current?snapshot.context:null
    if(context)lastMeasurement.current=measurement
    let notice=missing?'No runtime context exists yet. Send a message before compacting.'
      ...
      :phase==='busy'?'This conversation is busy. Wait for the active work to finish.'
      :phase==='unavailable'?'Context compaction is unavailable. Your conversation is preserved; check status or retry later.'
      :phase==='unknown'?'Compaction status is unconfirmed. Check status before continuing.'
      :snapshot.status==='missing'?'No runtime context is available for this conversation yet.'
      :snapshot.status==='unavailable'?'Context information is currently unavailable.':''
    ...
    publish({context,phase,notice,historyUnknown,...(snapshot.model?{observedCapacity:snapshot.model.contextWindow}:{}),...(!historyUnknown?{recoveryNotice:''}:{})})
  },[persist,publish])

  const refresh=useCallback(async(force=false)=>{
    if(mutation.current)return
    if(query.current)return query.current
    ...
    const run=(async()=>{
      try {
        ...
        if(at===generation.current)publish({context:null,...(pending.current?{phase:'unknown',notice:'Compaction status could not be checked. Your conversation is preserved; check status again.'}:{})})
      }finally {clearTimeout(timer);requests.current.delete(controller);if(at===generation.current)query.current=null}
    })()
    query.current=run
    return run
  },[accept,publish])
  ...
  useEffect(()=>{
    generation.current+=1;requests.current.forEach(controller=>controller.abort());requests.current.clear();query.current=null;mutation.current=false;lastQuery.current=0
    ...
    const modelChanged=priorRuntime.current!==runtimeKey && (Boolean(priorRuntime.current) || inspected.current)
    priorRuntime.current=runtimeKey
    if(pending.current || modelChanged)void refresh(true)
    return ()=>{generation.current+=1;requests.current.forEach(controller=>controller.abort());requests.current.clear();query.current=null}
    // A pending identity is restored only when changing sessions, not on autosave.
  },[chatId,runtimeKey,publish,refresh])

  useEffect(()=>{
    if(view.chatId!==chatId || view.runtimeKey!==runtimeKey || !['running','busy'].includes(view.phase))return
    const timer=setTimeout(()=>void refresh(true),2000)
  ...
  const compact=useCallback(async()=>{
    ...
    generation.current+=1;requests.current.forEach(controller=>controller.abort());requests.current.clear();query.current=null
    ...
    try {
      ...
      if(!snapshot && [400,401,403,404,409,412,413,423,429].includes(response.status)) {
        ...
        publish({phase:response.status===423?'busy':'failed',notice:response.status===423
          ?'This conversation is busy. No compaction was started; wait for the active work to finish.'
      ...
      requery=true
      if(at===generation.current)publish({context:null,phase:'unknown',notice:'Compaction was not confirmed. Your conversation is preserved; check status before retrying.'})
      return false
